import { GoogleGenAI, Part } from '@google/genai';
import { DEFAULT_TEXT_MODEL } from '../config/aiModels';

export type KoStudyAIResponse = {
  text: string;
  model: string;
  provider: 'server' | 'byok';
  quota?: { used: number; remaining: number };
};

export async function callKoStudyServerAI(args: {
  prompt?: string;
  contents?: Array<{ role: 'user' | 'model'; parts: Part[] }>;
  model?: string; // Backward-compatible input; shared server ignores caller model choice.
  task?: string;
}): Promise<KoStudyAIResponse> {
  const { model: _ignoredModel, ...serverArgs } = args;
  const response = await fetch('/.netlify/functions/gemini', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(serverArgs),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error || 'KoStudy server AI request failed');
  return { text: data.text || '', model: data.model || DEFAULT_TEXT_MODEL, provider: 'server', quota: data.quota };
}

export async function callGeminiWithUserKey(args: {
  apiKey: string;
  contents: Array<{ role: 'user' | 'model'; parts: Part[] }>;
  model?: string;
}): Promise<KoStudyAIResponse> {
  const genAI = new GoogleGenAI({ apiKey: args.apiKey });
  const model = args.model || DEFAULT_TEXT_MODEL;
  const result = await genAI.models.generateContent({ model, contents: args.contents });
  return { text: result.text || '', model, provider: 'byok' };
}
