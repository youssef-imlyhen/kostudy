...
export default function CategoryScreen() {
  const navigate = useNavigate(); const { categoryId = '' } = useParams(); const { setCurrentQuiz } = useQuiz(); const [progress] = useLocalStorage<Progress>('quizProgress', {}); const [lessonProgress] = useLocalStorage<LessonProgressState>('lessonProgress', {}); const { getQuestionsByCategory } = useQuestions(); const { getLessonsForCategory } = useLessons(); const { t } = useLanguage(); const questions = getQuestionsByCategory(categoryId); const relatedLessons = getLessonsForCategory(categoryId);
