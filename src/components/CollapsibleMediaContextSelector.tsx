...
interface CollapsibleMediaContextSelectorProps {
  value: MediaContext;
...
const CollapsibleMediaContextSelector: React.FC<CollapsibleMediaContextSelectorProps> = ({
  value,
  onChange,
  disabled = false,
  className = '',
  ...
  const getContextSummary = () => {
    if (value.type === 'none') return 'No Media Context';
    if (value.type === 'youtube' && value.url) return `YouTube: ${value.url.substring(0, 50)}...`;
    if (value.file) return `${MEDIA_CONTEXT_LABELS[value.type]}: ${value.file.name}`;
    return `${MEDIA_CONTEXT_LABELS[value.type]} Context`;
  };
  ...
  return (
    <div className={className}>
      ...
      <button
        ...
        disabled={disabled}
        className="w-full flex items-center justify-between p-3 bg-base-200 hover:bg-base-300 rounded-lg transition-colors duration-200 disabled:opacity-50"
      >
          ...
          {value.type === 'youtube' && (
            <div>
              ...
              <p className="text-xs text-base-content/60 mt-1">
                Enter a valid YouTube video URL (supports youtu.be links)
              </p>
            </div>
          )}
...
export default CollapsibleMediaContextSelector;