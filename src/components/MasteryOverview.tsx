    ...
    <section className="px-4 mb-8">
      <div className="rounded-3xl border border-base-300 bg-base-100 p-5 shadow-sm sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-primary">Concept mastery</p>
            <h2 className="mt-1 text-xl font-bold">What is becoming durable?</h2>
            <p className="mt-1 text-sm text-base-content/60">{practiced} practiced · {strong} strong · unseen concepts stay unscored.</p>
          </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {concepts.slice(0, 8).map((concept) => {
            ...
            return (
              <Link key={concept.id} to={lessonId ? `/lessons/${lessonId}` : '/categories'} className="rounded-2xl bg-base-200/55 p-4 transition hover:bg-base-200">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-semibold">{concept.label}</h3>
                    <p className="mt-1 text-xs text-base-content/55">{statusLabel[concept.status]}{concept.dueQuestions ? ` · ${concept.dueQuestions} due` : ''}</p>
                  </div>
