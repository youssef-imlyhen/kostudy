...
export default function EntropyCompressionLab() {
  ...
  const result = useMemo(() => {
    ...
    const probabilities = rawTotal > 0
      ? weights.map((value) => value / rawTotal)
    ...
    const sample = Array.from({ length: sampleSize }, (_, index) => {
      ...
      const symbolIndex = cumulative.findIndex((limit) => value <= limit);
      return symbols[symbolIndex >= 0 ? symbolIndex : symbols.length - 1];
    });
  ...
  return <div className="min-w-0 max-w-full space-y-5 overflow-x-hidden">
    ...
    <div className="min-w-0 rounded-2xl border border-base-300 bg-base-100 p-4"><div className="mb-2 text-xs font-bold uppercase tracking-wider text-primary">Deterministic sample</div><div className="break-all font-mono text-sm leading-6">{result.sample.join('')}</div><div className="mt-3 flex flex-wrap gap-2">{symbols.map((symbol, index) => <span key={symbol} className="badge badge-outline">{symbol}: {result.counts[index]}</span>)}</div></div>
    ...
    <div className="flex flex-wrap gap-2"><button type="button" className="btn btn-ghost btn-sm" onClick={() => preset([25, 25, 25, 25])}>Uniform</button><button type="button" className="btn btn-ghost btn-sm" onClick={() => preset([70, 20, 8, 2])}>Highly predictable</button><button type="button" className="btn btn-ghost btn-sm" onClick={() => preset([48, 48, 2, 2])}>Two common symbols</button></div>
