...
export default function ExponentialGrowthLab() {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 sm:grid-cols-3"><label className="space-y-2"><span className="flex justify-between text-sm font-medium"><span>Starting amount</span><span>{start}</span></span><input className="range range-primary range-sm" type="range" min="2" max="50" value={start} onChange={(event) => setStart(Number(event.target.value))} /></label><label className="space-y-2"><span className="flex justify-between text-sm font-medium"><span>Growth each step</span><span>{rate}%</span></span><input className="range range-secondary range-sm" type="range" min="1" max="60" value={rate} onChange={(event) => setRate(Number(event.target.value))} /></label><label className="space-y-2"><span className="flex justify-between text-sm font-medium"><span>Number of steps</span><span>{steps}</span></span><input className="range range-accent range-sm" type="range" min="3" max="24" value={steps} onChange={(event) => setSteps(Number(event.target.value))} /></label></div>
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="rounded-2xl bg-base-200/70 p-3"><div className="text-xs text-base-content/55">Linear final</div><div className="mt-1 text-xl font-black">{final.linear.toFixed(1)}</div></div><div className="rounded-2xl bg-base-200/70 p-3"><div className="text-xs text-base-content/55">Exponential final</div><div className="mt-1 text-xl font-black">{final.exponential.toFixed(1)}</div></div><div className="rounded-2xl bg-base-200/70 p-3"><div className="text-xs text-base-content/55">Gap</div><div className="mt-1 text-xl font-black">{(final.exponential - final.linear).toFixed(1)}</div></div><div className="rounded-2xl bg-base-200/70 p-3"><div className="text-xs text-base-content/55">Approx. doubling</div><div className="mt-1 text-xl font-black">{Number.isFinite(doubling) ? `${doubling.toFixed(1)} steps` : 'Never'}</div></div></div>
