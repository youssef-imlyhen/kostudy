...
function intendedAction(strategy: Strategy, round: number, opponentHistory: Action[], randomSeed: number): Action {
  if (strategy === 'cooperate') return 'C';
  if (strategy === 'defect') return 'D';
  if (strategy === 'random') return deterministicRandom(round + randomSeed) > 0.5 ? 'C' : 'D';
  if (round === 0) return 'C';
...
export default function RepeatedGameLab() {
  ...
  const game = useMemo(() => {
    ...
    for (let round = 0; round < rounds; round += 1) {
      let actionA = intendedAction(strategyA, round, historyB, 91);
      let actionB = intendedAction(strategyB, round, historyA, 191);
  ...
  return <div className="min-w-0 max-w-full space-y-5 overflow-x-hidden">
    ...
    <div className="lab-grid lab-grid-compact text-center text-xs"><div className="rounded-xl bg-success/10 p-2">C/C → 3, 3</div><div className="rounded-xl bg-warning/10 p-2">D/C → {temptation}, 0</div><div className="rounded-xl bg-warning/10 p-2">C/D → 0, {temptation}</div><div className="rounded-xl bg-error/10 p-2">D/D → 1, 1</div></div>
