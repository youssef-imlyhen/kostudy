...
export default function SignalDetectionLab() {
  ...
  const metrics = useMemo(() => {
    ...
    const positiveValue = positiveTotal > 0 ? truePositive / positiveTotal : 0;
  ...
  return <div className="min-w-0 max-w-full space-y-5 overflow-x-hidden">
    <div className="rounded-3xl bg-base-200/70 p-4 sm:p-5">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><div className="text-sm font-bold">Noise and signal overlap</div><div className="text-xs text-base-content/50">The vertical line is the current response criterion</div></div><div className="flex gap-3 text-xs"><span className="font-semibold text-primary">Solid: noise</span><span className="font-semibold text-secondary">Dashed: signal</span></div></div>
      <svg viewBox="0 0 100 100" className="h-56 w-full overflow-visible rounded-2xl bg-base-100" role="img" aria-labelledby="signal-chart-title signal-chart-description">
        <title id="signal-chart-title">Noise and signal distributions</title>
        <desc id="signal-chart-description">A solid noise curve centered at zero, a dashed signal curve shifted by the selected separation, and a vertical response criterion.</desc>
        <path d={pathFor(0)} fill="none" stroke="currentColor" strokeWidth="1.6" className="text-primary" />
        <path d={pathFor(separation)} fill="none" stroke="currentColor" strokeWidth="1.6" strokeDasharray="4 2" className="text-secondary" />
    ...
    <div className="lab-grid lab-grid-compact">
      {[['Hit rate', metrics.hitRate], ['False alarms', metrics.falseAlarmRate], ['Miss rate', metrics.missRate], ['True among positives', metrics.positiveValue]].map(([label, raw]) => <div key={String(label)} className="rounded-2xl border border-base-300 bg-base-100 p-3 text-center"><div className="text-xs text-base-content/55">{label}</div><div className="mt-1 text-xl font-black">{(Number(raw) * 100).toFixed(0)}%</div></div>)}
    </div>

    <div className="lab-grid lab-grid-medium">
      <label className="space-y-2"><span className="flex justify-between text-sm"><span>Signal separation</span><span>{separation.toFixed(1)}</span></span><input className="range range-secondary range-sm" type="range" min="0.2" max="3.5" step="0.1" value={separation} aria-valuetext={`${separation.toFixed(1)} standard deviations of separation`} onChange={(event) => setSeparation(Number(event.target.value))} /></label>
      <label className="space-y-2"><span className="flex justify-between text-sm"><span>Response criterion</span><span>{criterion.toFixed(1)}</span></span><input className="range range-warning range-sm" type="range" min="-1" max="4" step="0.1" value={criterion} aria-valuetext={`${criterion.toFixed(1)} standard deviations`} onChange={(event) => setCriterion(Number(event.target.value))} /></label>
      <label className="space-y-2"><span className="flex justify-between text-sm"><span>Signal prevalence</span><span>{prevalence}%</span></span><input className="range range-primary range-sm" type="range" min="1" max="70" value={prevalence} aria-valuetext={`${prevalence} percent prevalence`} onChange={(event) => setPrevalence(Number(event.target.value))} /></label>
    </div>
