...
export default function CircuitLab() {
  ...
  return <div className="min-w-0 max-w-full space-y-5 overflow-x-hidden">
    <div className="lab-grid lab-grid-wide">
      <div className="rounded-3xl bg-base-200/70 p-4"><svg viewBox="0 0 120 70" className="h-64 w-full rounded-2xl bg-base-100" role="img" aria-labelledby="circuit-title circuit-description">
        <title id="circuit-title">{arrangement === 'series' ? 'Series circuit' : 'Parallel circuit'}</title>
        <desc id="circuit-description">A {voltage} volt source connected to resistors R1 at {r1} ohms and R2 at {r2} ohms in {arrangement}.</desc>
      ...
      <div className="lab-grid lab-grid-compact"><div className="rounded-2xl border border-primary/20 bg-primary/5 p-4"><div className="text-xs text-base-content/55">Equivalent resistance</div><div className="mt-1 text-3xl font-black">{result.equivalent.toFixed(2)} Ω</div></div><div className="rounded-2xl border border-base-300 bg-base-100 p-4"><div className="text-xs text-base-content/55">Total current</div><div className="mt-1 text-3xl font-black">{result.totalCurrent.toFixed(2)} A</div></div><div className="rounded-2xl border border-base-300 bg-base-100 p-4"><div className="text-xs text-base-content/55">Total power</div><div className="mt-1 text-3xl font-black">{result.power.toFixed(1)} W</div></div></div>
    </div>
    <div className="lab-grid lab-grid-medium"><label className="space-y-2"><span className="text-sm font-semibold">Arrangement</span><select className="select select-bordered select-sm w-full" value={arrangement} onChange={(event) => setArrangement(event.target.value as Arrangement)}><option value="series">Series</option><option value="parallel">Parallel</option></select></label><label className="space-y-2"><span className="flex justify-between text-sm"><span>Source voltage</span><span>{voltage} V</span></span><input className="range range-primary range-sm" type="range" min="1" max="24" value={voltage} aria-valuetext={`${voltage} volts`} onChange={(event) => setVoltage(Number(event.target.value))} /></label><label className="space-y-2"><span className="flex justify-between text-sm"><span>R1</span><span>{r1} Ω</span></span><input className="range range-warning range-sm" type="range" min="2" max="40" value={r1} aria-valuetext={`${r1} ohms`} onChange={(event) => setR1(Number(event.target.value))} /></label><label className="space-y-2"><span className="flex justify-between text-sm"><span>R2</span><span>{r2} Ω</span></span><input className="range range-secondary range-sm" type="range" min="2" max="40" value={r2} aria-valuetext={`${r2} ohms`} onChange={(event) => setR2(Number(event.target.value))} /></label></div>
    <div className="lab-grid lab-grid-medium"><div className="rounded-2xl border border-warning/20 bg-warning/5 p-4"><div className="text-sm font-bold">{arrangement === 'series' ? 'R1 resistor' : 'R1 branch'}</div><div className="mt-2 text-sm">Current {result.branch1.toFixed(2)} A · Voltage {result.v1.toFixed(2)} V</div></div><div className="rounded-2xl border border-secondary/20 bg-secondary/5 p-4"><div className="text-sm font-bold">{arrangement === 'series' ? 'R2 resistor' : 'R2 branch'}</div><div className="mt-2 text-sm">Current {result.branch2.toFixed(2)} A · Voltage {result.v2.toFixed(2)} V</div></div></div>
