...
export default function StickSlipLab() {
  ...
  const eventsThroughCursor = simulation.events.filter((event) => event.step <= time);
  const recentEvent = eventsThroughCursor[eventsThroughCursor.length - 1];

  return <div className="min-w-0 max-w-full space-y-5 overflow-x-hidden">
    <div className="lab-grid lab-grid-wide">
      <div className="rounded-3xl bg-base-200/70 p-4"><div className="mb-3"><div className="text-sm font-bold">Stored stress through time</div><div className="text-xs text-base-content/50">Gradual loading, sudden release</div></div><svg viewBox="0 0 100 100" className="h-64 w-full rounded-2xl bg-base-100" role="img" aria-labelledby="stick-slip-title stick-slip-description"><title id="stick-slip-title">Stored stress through time</title><desc id="stick-slip-description">Stress rises gradually and drops abruptly at release events. A vertical line marks the time cursor and a horizontal dashed line marks the baseline failure threshold.</desc><path d={path} fill="none" stroke="currentColor" strokeWidth="1.5" className="text-primary" /><line x1={time} x2={time} y1="8" y2="94" stroke="currentColor" strokeWidth="1" strokeDasharray="3 2" className="text-warning" /><line x1="0" x2="100" y1={92 - threshold / maxStress * 78} y2={92 - threshold / maxStress * 78} stroke="currentColor" strokeWidth="0.8" strokeDasharray="2 2" className="text-error/50" /><text x="2" y={Math.max(7, 89 - threshold / maxStress * 78)} fontSize="3.2" className="fill-error">baseline threshold</text></svg></div>
      <div className="lab-grid lab-grid-compact"><div className="rounded-2xl border border-primary/20 bg-primary/5 p-4"><div className="text-xs text-base-content/55">Current stress</div><div className="mt-1 text-3xl font-black">{current.toFixed(1)}</div></div><div className="rounded-2xl border border-base-300 bg-base-100 p-4"><div className="text-xs text-base-content/55">Events through cursor</div><div className="mt-1 text-3xl font-black">{eventsThroughCursor.length}</div></div><div className="rounded-2xl border border-base-300 bg-base-100 p-4"><div className="text-xs text-base-content/55">Latest drop before cursor</div><div className="mt-1 text-3xl font-black">{recentEvent ? recentEvent.drop.toFixed(1) : '—'}</div></div></div>
    </div>
