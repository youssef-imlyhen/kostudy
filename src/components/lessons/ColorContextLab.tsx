...
export default function ColorContextLab() {
  ...
  return <div className="min-w-0 max-w-full space-y-5 overflow-x-hidden">
    ...
    <div className="flex flex-wrap gap-2"><button type="button" className="btn btn-primary btn-sm" onClick={() => setReveal((value) => !value)}>{reveal ? 'Hide physical values' : 'Reveal identical values'}</button><button type="button" className="btn btn-ghost btn-sm" onClick={() => { setLeftBackground(2); setRightBackground(95); }}>Use extreme contexts</button><button type="button" className="btn btn-ghost btn-sm" onClick={() => { setLeftBackground(50); setRightBackground(50); }}>Match contexts</button></div>
