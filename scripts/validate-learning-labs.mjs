...
const contracts=[
 ...
 ['shared action button semantics',source.actionButton,['type="button"','aria-label={label}','aria-hidden="true"','const canCollapseToIcon = responsive && Boolean(icon);',"canCollapseToIcon ? 'hidden sm:inline' : undefined"]],
 ['JavaScript reduced-motion hook',source.reducedMotionHook,["'(prefers-reduced-motion: reduce)'","mediaQuery.addEventListener('change', updatePreference)",'mediaQuery.addListener(updatePreference)']],
 ['equilibrium reduced-motion completion',source.equilibrium,['usePrefersReducedMotion','prefersReducedMotion&&playing','setStep(120)',"prefersReducedMotion?'Show equilibrium'"]],
 ['orbit reduced-motion stepping',source.orbit,['usePrefersReducedMotion','if (!playing || prefersReducedMotion) return','setPhase((value) => (value + 0.125) % 1)',"prefersReducedMotion?'Advance orbit'"]],
 ['polyrhythm reduced-motion audio',source.polyrhythm,['usePrefersReducedMotion','if(!prefersReducedMotion)setPhase(next)','visual sweep stays still for reduced motion']],
 ['projectile reduced-motion landing',source.projectile,['usePrefersReducedMotion','setPhase(1)',"prefersReducedMotion?'Show landing'"]],
 ['sorting reduced-motion completion',source.sorting,['usePrefersReducedMotion','setIndex(frames.length-1)',"prefersReducedMotion?'Finish sort'"]],
